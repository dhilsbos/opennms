...
public class MinionStatusTrackerTest {
    ...
    private static final String MINION_HEARTBEAT = MinionStatusTrackerImpl.MINION_HEARTBEAT;
    private static final String MINION_RPC = MinionStatusTrackerImpl.MINION_RPC;

    private static final long PERIOD = 2 * 30 * 1000;

    private MinionStatusTrackerImpl m_tracker;
    ...
    public void setUp() throws Exception {
        ...
        m_tracker = new MinionStatusTrackerImpl();
    ...
    public void testOtherServiceFails() throws Exception {
        ...
        Event e = new EventBuilder(EventConstants.OUTAGE_CREATED_EVENT_UEI, FOREIGN_SOURCE)
                .setNodeid(1)
                .setService("WontYouTakeMeToFunkyTown")
                .getEvent();
    ...
    public void testMinionResponsive() throws Exception {
        ...
        Event e = new EventBuilder(EventConstants.OUTAGE_RESOLVED_EVENT_UEI, FOREIGN_SOURCE)
                .setNodeid(1)
                .setService(MINION_HEARTBEAT)
                .getEvent();
        m_tracker.onOutageEvent(e);
        ...
        e = new EventBuilder(EventConstants.OUTAGE_RESOLVED_EVENT_UEI, FOREIGN_SOURCE)
                .setNodeid(1)
                .setService(MINION_RPC)
                .getEvent();
        m_tracker.onOutageEvent(e);
    ...
    public void testRefresh() throws Exception {
        ...
        final String foreignIdB = UUID.randomUUID().toString();
        final OnmsNode nodeB = getNode(2, FOREIGN_SOURCE, foreignIdB, "MinionLocB");
        final OnmsMinion minionB = getMinion(nodeB);

        final String foreignIdC = UUID.randomUUID().toString();
        final OnmsNode nodeC = getNode(3, FOREIGN_SOURCE, foreignIdC, "MinionLocC");
        final OnmsMinion minionC = getMinion(nodeC);

        final String foreignIdD = UUID.randomUUID().toString();
        final OnmsNode nodeD = getNode(4, FOREIGN_SOURCE, foreignIdD, "MinionLocD");
        final OnmsMinion minionD = getMinion(nodeD);

        final Date tooOld = new Date(System.currentTimeMillis() - (3 * PERIOD));
        final List<OnmsOutage> outages = Arrays.asList(
                                                       createOutage(new Date(), null, nodeA, MINION_HEARTBEAT), // nodeA heartbeat down
                                                       createOutage(new Date(), null, nodeA, MINION_RPC), // nodeA RPC down
                                                       createOutage(tooOld, tooOld, nodeB, MINION_HEARTBEAT), // nodeB heartbeat too old
                                                       createOutage(new Date(), new Date(), nodeB, MINION_RPC), // nodeB RPC up
                                                       createOutage(new Date(), new Date(), nodeC, MINION_HEARTBEAT), // nodeC heartbeat up
                                                       createOutage(new Date(), new Date(), nodeC, MINION_RPC) // nodeC RPC up
                );
        ...
        when(m_outageDao.findMatching(any(Criteria.class))).thenReturn(outages);
        ...
        Event e = new EventBuilder(EventConstants.OUTAGE_RESOLVED_EVENT_UEI, nodeB.getForeignSource())
                .setNodeid(nodeB.getId())
                .setService(MINION_HEARTBEAT)
                .getEvent();
        m_tracker.onOutageEvent(e);
        assertTrue("node B should now be up", m_tracker.getStatus(foreignIdB).isUp(PERIOD));

        verify(m_minionDao, times(1)).saveOrUpdate(minionB);
    }
    ...
    public void testRefreshNoOutages() throws Exception {
        ...
        Event e = new EventBuilder(EventConstants.OUTAGE_RESOLVED_EVENT_UEI, nodeA.getForeignSource())
                .setNodeid(nodeA.getId())
                .setService(MINION_HEARTBEAT)
                .setTime(heartbeatUpDate)
                .getEvent();
        m_tracker.onOutageEvent(e);
        ...
        e = new EventBuilder(EventConstants.OUTAGE_RESOLVED_EVENT_UEI, nodeA.getForeignSource())
                .setNodeid(nodeA.getId())
                .setService(MINION_RPC)
                .setTime(rpcUpDate)
                .getEvent();
        ...
        when(m_outageDao.findMatching(any(Criteria.class))).thenReturn(Collections.emptyList());
        ...
        assertEquals("node A heartbeat up should be as of " + heartbeatUpDate, statusA.getHeartbeatStatus().lastSeen(), heartbeatUpDate);
        ...
        assertEquals("node A RPC up should be as of " + rpcUpDate, statusA.getRpcStatus().lastSeen(), rpcUpDate);
    ...
    private OnmsMinion getMinion(final OnmsNode node) {
        ...
        final OnmsMinion minion = m_minions.getOrDefault(nodeId, new OnmsMinion(foreignId, location, null, new Date()));
